...
namespace VULKAN
{
	...
	PipelineReader::PipelineReader(MyVulkanDevice& device):myVulkanDevice(device)
	{
	}
	...
	void PipelineReader::UIPipelineDefaultConfigInfo(PipelineConfigInfo& configInfo)
	{
		...
		configInfo.colorBlendAttachment.srcAlphaBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;   // Optional
		...
		VkPipelineDepthStencilStateCreateInfo depthStencil{};
		configInfo.depthStencilInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
		configInfo.depthStencilInfo.depthTestEnable = VK_FALSE;  // Enable depth test
		configInfo.depthStencilInfo.depthWriteEnable = VK_FALSE; // Enable depth write
		configInfo.depthStencilInfo.depthCompareOp = VK_COMPARE_OP_NEVER; // Depth comparison operation
		configInfo.depthStencilInfo.depthBoundsTestEnable = VK_FALSE;    // Depth bounds test: not used here
		configInfo.depthStencilInfo.stencilTestEnable = VK_FALSE;
	...
	void PipelineReader::CreateShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule, MyVulkanDevice& device)
	{
		VkShaderModuleCreateInfo createInfo{};
		createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
		createInfo.codeSize = code.size();
